...
export async function wordsmithLoader({
  ...
  if (params.mode === "daily") {
    ...
    if (savedResult) {
      const { foundWords, puzzle, completionTime } = JSON.parse(savedResult);
      return { puzzle, foundWords, completionTime };
    }
...
export function useWordsmithGame() {
  ...
  const {
    ...
    currentStreak,
    usedPuzzles,
    completionTime: restoredCompletionTime,
  ...
  const [completionTime, setCompletionTime] = useState<number | null>(
    restoredCompletionTime ?? null,
  );
  ...
  useEffect(() => {
    ...
    if (params.mode === "daily" || (params.mode === "endless" && !isSolved)) {
      setTimeout(() => setShowModal(true), 1000);
    }
  }, [readOnly]);
